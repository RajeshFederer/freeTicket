...
let persistIntents = () => {
    ...
    .matches('bookTicket', [(session, args,callback) =>{
        ...
        console.log('ENTITIES ', dateOfTravel, noOfTickets, toLocation, travelClass, session.userData);
        if (noOfTickets) {
    ...
    (session, results, callback) => {
        ...
        if (session.userData.toLocation != undefined || session.userData.toLocation != null){
            console.log('1', session.userData.toLocation);
            return callback();
        } else {
    ...
    .matches('flights',[(session, args,callback) =>{
        ...
        if(results.response){
            session.send('Flight Ticket booking Successful. \n'+ 
            session.userData.fromLocation + " to " + session.userData.toLocation +  '\n ' +
            session.userData.flightName + ' \n ' +
            ' Boarding Time: ' + moment(session.userData.dateOfTravel.resolution.start).format('DD-MM-YYYY') + '10:30' + '\n' +
