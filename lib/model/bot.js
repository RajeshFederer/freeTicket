    ...
    .matches('bookTicket', [(session, args,callback) =>{
        ...
        if (session.dialogData.travelClass){
            ...
            msg.attachments([
                new builder.HeroCard(session)
                    ...
                    .buttons([
                        builder.CardAction.postBack(session, "Economy", "Economy"),
                        builder.CardAction.postBack(session, "Business", "Business"),
                        builder.CardAction.postBack(session, "Premium Economy", "Premium Economy"),
                        builder.CardAction.postBack(session, "First Class", "First Class")
                    ])
            ]);
            session.send(msg);
        }
        ...
        for(var flight in flightList){
            attachments.push (new builder.HeroCard(session)
                ...
                .buttons([
                    builder.CardAction.imBack(session, "Select", flightList[flight].flightName)
                ]));
        }
