import { response } from './C:/Users/39850/AppData/Local/Microsoft/TypeScript/2.6/node_modules/@types/spdy';
...
let persistIntents = () => {
    ...
    .matches('flights',[(session, args,callback) =>{
        console.log('OOO ', session.message.text);
        console.log('IN', session.dialogData);
        session.send('FINAL').endDialog();
        session.dialogData.flightName = session.message.text;
        builder.Prompts.text(session, 'Please tell your Email Id to recieve ticket');
    }, (session, results, callback) => {
        session.dialogData.contactEmailId = results.response;
        builder.Prompts.confirm(session, 'Your Payable Amount is Rs.4000. Please Confirm it');
    },  (session, results, callback) => {
        console.log('result', results.response)
            session.send('Your Ticket has been booked'+'\n'+ 'Flight Name : '+ session.dialogData.flightName + '\n' +
                         'Date :' + session.dialogData.dateOfTravel );
    }])
