...
let persistIntents = () => {
    ...
    .matches('bookTicket', [(session, args,callback) =>{
        ...
        console.log(args.entities+'DATE '+ JSON.stringify(dateOfTravel) + ' noOfTickets :'+ JSON.stringify(noOfTickets) + 'DESTINATION '+ JSON.stringify(destinationLocation) + 'ORIGIN '+ JSON.stringify(originLocation) + 'TRAVEL CLASS '+ travelClass);
        session.userData = {};
        if (noOfTickets) {
            session.userData.noOfTickets = noOfTickets.entity;
        }
        ...
        if (dateOfTravel){
            session.userData.dateOfTravel = dateOfTravel.entity;
            return callback();
        } else {
