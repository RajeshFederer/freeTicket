...
let persistIntents = () => {
    ...
    .matches('Greeting', (session) => {
        if(!session.userData || !session.userData.oldUser){
    ...
    .matches('bookTicket', [(session, args,callback) =>{
        ...
        console.log(dateOfTravel, noOfTickets, toLocation, travelClass);
        session.userData = {oldUser : true};
        if (noOfTickets) {
    ...
    (session, results, callback) => {
       ...
       /* if (results.response) {
        ...
        session.send(msg);
    }]).beginDialogAction('Greetings' ,{
        matches: /^exit$|^cancel$|^goodbye$/i,
        onSelectAction: (session, args, next) => {
            session.send('I am resetting now.')
        }
    })
